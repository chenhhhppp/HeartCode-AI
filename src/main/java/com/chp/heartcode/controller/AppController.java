...
import com.chp.heartcode.model.dto.app.*;
...
public class AppController {
    ...
    public Flux<ServerSentEvent<String>> chatToGenCode(@RequestParam Long appId,
        ...
        User loginUser = userService.getLoginUser(request);
        // 调用服务生成代码（流式）
        reactor.core.publisher.Flux<String> contentFlux = appService.chatToGenCode(appId, message, loginUser);
        // 转换为 ServerSentEvent 流
        return contentFlux
                .map(chunk -> {
                    // 将内容包装成JSON对象（用 HashMap 避免 Map.of 在旧 JDK 下编译失败）
                    Map<String, String> wrapper = new HashMap<>();
                    wrapper.put("data", chunk);
                ...
                .onErrorResume(error -> {
                    // 捕获流式处理过程中的异步异常，转为 SSE 错误事件
                    // 防止异常传播到 GlobalExceptionHandler（它返回 BaseResponse，
                    // 但响应 Content-Type 已是 text/event-stream，无法转换）
                    log.error("SSE 流式生成代码失败: appId={}, message={}", appId, error.getMessage(), error);
                    Map<String, String> errorWrapper = new HashMap<>();
                    errorWrapper.put("data", "生成失败：" + error.getMessage());
    ...
    public BaseResponse<Long> addApp(@RequestBody AppAddRequest appAddRequest, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(CodeGenTypeEnum.getEnumByValue(codeGenType) == null,
                ErrorCode.PARAMS_ERROR, "代码生成类型不合法");
